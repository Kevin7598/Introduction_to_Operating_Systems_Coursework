...
auto ComplexQuery::initCondition(const Table &table)
  ...
  for (auto &cond : condition) {
    if (cond.field == "KEY") {
      ...
      cond.valueParsed = static_cast<Table::ValueType>(std::stoi(cond.value));
      int operation = 0;
      try {
        operation = opmap.at(cond.op);
...
auto ComplexQuery::evalCondition(const Table::Object &object) const -> bool {
  bool ret = true;
